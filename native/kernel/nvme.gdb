...
define nvme-get-active-cmds
	...
	while ($_itr != $_head)
		...
		set $_cmdPtr = (vmk_ScsiCommand*)$_cmdInfo->cmdPtr
		...
		if ($_cmdPtr == NULL)
			...
			printf "\tbase:%p cmdPtr:(null)\n", $_cmdBase
		end

		if ($_cmdPtr != NULL)
			set $_sgArray = ($_cmdPtr->sgIOArray)

			printf "\tbase:%p cmdPtr:%p (I:%p SN:0x%llx W:%d) ", $_cmdBase, $_cmdPtr, $_cmdPtr->cmdId.initiator, $_cmdPtr->cmdId.serialNumber, $_cmdPtr->worldId

			if ($_cmdPtr->isReadCdb)
				printf "RD "
			else
				printf "WR "
			end

			printf "LBA:0x%llx LBC:%d | OPC:%xh PRP0:0x%llx PRP1:0x%llx SLBA:0x%llx NLBA:%d\n", $_cmdPtr->lba, $_cmdPtr->lbc, $_nvmeCmd->header.opCode, $_nvmeCmd->header.prp[0].addr, $_nvmeCmd->header.prp[1].addr, $_nvmeCmd->cmd.read.startLBA, $_nvmeCmd->cmd.read.numLBA
		end
