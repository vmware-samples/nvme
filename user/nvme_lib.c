...
/**
  ...
  */
int
Nvme_CreateNamespace_IDT(struct nvme_handle *handle, int ns, __u32 snu, __u32 nnu)
{
   int rc = 0;
   struct usr_io uio;
...
int Nvme_FWDownload(struct nvme_handle *handle, int slot,  unsigned char *rom_buf, int rom_size)
{
   ...
   for (offset = 0; offset < rom_size; offset += NVME_MAX_XFER_SIZE) {
      ...
      uio.timeoutUs = ADMIN_TIMEOUT;
      uio.cmd.cmd.firmwareDownload.numDW = (size / sizeof(__u32)) - 1;
      uio.cmd.cmd.firmwareDownload.offset = offset / sizeof(__u32);
      uio.addr = (__u32)chunk;
      uio.length = size;
...
int Nvme_FWActivate(struct nvme_handle *handle, int slot, int action)
{
   ...
   if (uio.comp.SCT == SF_SCT_CMD_SPC_ERR &&
   ...
   if (uio.comp.SCT!= SF_SCT_GENERIC  &&
       uio.comp.SC!= SF_SC_SUCCESS) {
      /* Fail to activate NVMe firmware: unkonwn failure. */
      return -EIO;
   }
...
int Nvme_FormatNvm(struct nvme_handle *handle, int ses, int pil, int pi, int ms, int lbaf, int ns)
{
   ...
   uio.namespace = ns;
   uio.timeoutUs = ADMIN_TIMEOUT * 2; /* Give Format NVM more time to run */

   rc = Nvme_AdminPassthru(handle, &uio);

   if (rc) {
      return rc;
   } else if (uio.status == VMK_NOT_SUPPORTED) {
      return -EINVAL;
   } else {
